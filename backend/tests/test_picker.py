...
"""Enough ratings to offer a card for every built-in quality and then one more."""
...
async def worn_in(client, run_jobs, size=5):
    """An account with an ordering across several bands, and its first prose written."""
...
async def test_anchor_pre_ticks_its_guess_until_the_owner_answers(owner, run_jobs, provider):
    ...
    provider.will_say(llm.SUGGEST_SYSTEM, qualities=["Pacing", "Ending"])
...
async def test_a_guess_is_not_a_constraint(owner, db, run_jobs, provider):
    ...
    """
    provider.will_say(llm.SUGGEST_SYSTEM, qualities=["Pacing"])
...
async def test_the_guess_is_shown_the_owners_criteria_answers(owner, run_jobs, provider):
    ...
    await worn_in(owner, run_jobs)
    landed = await rate(owner, LIBRARY[5], 3.0)
...
async def test_a_hollow_account_is_never_guessed_at(owner, db, run_jobs, provider):
    ...
    await rate(owner, LIBRARY[0], 3.0)
    await run_jobs()
