...
async def _advance(
    ...
    *,
    ...
    # The films a card from this landing could have named are the ones the next card will
    # want tags for, so this landing is what buys them - for the next placement, never for
    # this one. Precompute only: nothing above waits on the job this queues.
    await tags.schedule(
        db,
        queue,
        account.id,
        criteria_module.askable_films(entries, flow.context, flow.since),
        settings,
    )
