...
the list is simply the fully trusted films whose band has moved off it, plus the ones
that never had one because Letterboxd never saw them. A rating that wobbles back to its
synced value therefore drops off by itself: the difference the list is made of is gone,
and nothing has to notice.

*A placeholder position is not a judgment.* Provisional placements are held back until
they graduate, which is what makes the list empty immediately after an import rather than
a list of every film in the account. The owner should never be retyping a value Anchor
has not finished making up its own mind about.
...
from anchor.models import AccountFilm, Film, LifecycleState, Placement, PlacementTrust
...
async def mark_all_synced(account: CurrentAccount, db: DbSession) -> None:
    ...
    typed. A film that graduated between the read and this write joins the list again on
    the next read rather than being swept up silently.
    """
...
async def mark_synced(tmdb_id: int, account: CurrentAccount, db: DbSession) -> None:
    ...
    judgment to carry over at all - unrated, or a placement still settling - because
    there the write would invent a baseline out of a placeholder.
    """
    ...
    if found is None:
        raise ApiError(
            409,
            "nothing_to_sync",
            "That film has no settled rating to carry over yet.",
        )
...
async def _carriable(db: AsyncSession, account_id: uuid.UUID) -> list[tuple[AccountFilm, float]]:
    """Every film that has a value worth carrying over, paired with that value.

    Two exclusions, and they are the same exclusion said twice: a provisional placement is
    a position Anchor has not settled, and a slot the dividers cannot yet decide has no
    band at all. Neither is a rating the owner could type into Letterboxd.
    """
    ordering = await ordering_module.load(db, account_id)
    derived = ordering_module.bands_of(ordering, await bands.load(db, account_id))
    trusted = await _trusted(db, account_id)
    return [
        (trusted[film_id], band)
        for slot in ordering.slots
        for film_id in slot.film_ids
        if film_id in trusted and (band := derived[film_id]) is not None
    ]


async def _trusted(db: AsyncSession, account_id: uuid.UUID) -> dict[int, AccountFilm]:
    """The rated films whose position is a judgment rather than a placeholder."""
