...
A film's rating is which dividers its position sits between.

## Consequences

- Rank distance is meaningless on an ordinal sequence, so "closest centroid" is operationalized by owner judgments (sliver questions), never by counting films.
- Bands work without an anchor: dividers derive from judgments about ordinary films and persist if an anchor is retired.
- Dividers only move as a direct consequence of an owner judgment, preserving the never-changes-behind-your-back guarantee.
