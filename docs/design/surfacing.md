...
Consolidates wayfinder ticket [Surfacing and nudge cadence (#18)](https://github.com/KyleKDang/anchor/issues/18): every surfacing moment the design creates, placed under one global posture, recorded as [ADR 0011](../adr/0011-no-nagging-surfacing-policy.md).
The posture exists for trust: engagement conventions optimize for return visits, while Anchor optimizes for the owner's judgment staying unpressured, because comparisons made out of obligation are noise in the one layer that must stay honest.
...
- Provisional placements are held back until graduation, so the list is empty right after import and fills only as real judgments accumulate.
